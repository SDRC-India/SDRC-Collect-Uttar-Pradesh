...
import org.sdrc.collect.uphpmis.android.exception.EncryptionException;
...
import org.sdrc.collect.uphpmis.android.utilities.EncryptionUtils;
import org.sdrc.collect.uphpmis.android.utilities.FileUtils;
import org.sdrc.collect.uphpmis.android.utilities.EncryptionUtils.EncryptedFormInformation;
...
public class SaveToDiskTask extends AsyncTask<Void, String, SaveResult> {
    ...
    protected SaveResult doInBackground(Void... nothing) {
        ...
        //Ratikanta start
        if(Collect.getInstance().isFormDownloaded()){
        	updateInstanceDatabase(true, false);
        }else{
    ...
    private void updateInstanceDatabase(boolean incomplete, boolean canEditAfterCompleted) {
        ...
        if(Collect.getInstance().isFormDownloaded()){
        	values.put(InstanceColumns.STATUS, InstanceProviderAPI.STATUS_DOWNLOADED);
        }else{
