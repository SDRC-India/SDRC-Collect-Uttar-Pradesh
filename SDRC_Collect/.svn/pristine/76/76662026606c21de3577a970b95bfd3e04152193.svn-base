...
import org.sdrc.collect.uphpmis.android.listeners.InstanceUploaderListener;
...
import org.sdrc.collect.uphpmis.android.utilities.WebUtils;
...
public class InstanceUploaderTask extends AsyncTask<Long, Integer, InstanceUploaderTask.Outcome> {
	 ...
	 *         immediately.
	 */
	private boolean uploadOneSubmission(String urlString, String id, String instanceFilePath, Uri toUpdate,
			HttpContext localContext, Map<Uri, Uri> uriRemap, Outcome outcome) {
					...
					if (locations != null && locations.length == 1) {
						try {
							...
							if (u.getHost().equalsIgnoreCase(uNew.getHost())) {
								...
								outcome.mResults.put(id, fail + "Unexpected redirection attempt to a different host: "
										+ uNew.toString());
			...
			try {
				...
				if (responseCode != HttpStatus.SC_CREATED && responseCode != HttpStatus.SC_ACCEPTED) {
					if (responseCode == HttpStatus.SC_OK) {
						...
						// clear the cookies -- should not be necessary?
						Collect.getInstance().getCookieStore().clear();
						outcome.mResults.put(id, fail + response.getStatusLine().getReasonPhrase() + " (" + responseCode
								+ ") at " + urlString);
					}

					// Ratikanta
					else if (responseCode == HttpStatus.SC_NOT_ACCEPTABLE) {

						// This error will come when the form not found in the
						// server to this user.
						outcome.mResults.put(id, fail + "Unassigned form");
					} else {
						outcome.mResults.put(id,
								fail + response.getStatusLine().getReasonPhrase() + " (" + responseCode + ")");

						// The following error was being shown earlier, we
						// changed it to the above, except the URL.
						// outcome.mResults.put(id, fail +
		...
		synchronized (this) {
			if (mStateListener != null) {
				if (outcome.mAuthRequestingServer != null) {
					...
					Cursor results = null;
					try {
						results = Collect.getInstance().getContentResolver().query(InstanceColumns.CONTENT_URI, null,
	...
	public static void copyToBytes(InputStream input, OutputStream output, int bufferSize) throws IOException {
