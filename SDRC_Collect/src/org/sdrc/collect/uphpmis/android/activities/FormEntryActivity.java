...
import org.sdrc.collect.uphpmis.android.listeners.AdvanceToNextListener;
import org.sdrc.collect.uphpmis.android.listeners.FormLoaderListener;
import org.sdrc.collect.uphpmis.android.listeners.FormSavedListener;
import org.sdrc.collect.uphpmis.android.listeners.SavePointListener;
import org.sdrc.collect.uphpmis.android.logic.FormController;
import org.sdrc.collect.uphpmis.android.logic.FormController.FailedConstraint;
...
import org.sdrc.collect.uphpmis.android.tasks.FormLoaderTask;
import org.sdrc.collect.uphpmis.android.tasks.SavePointTask;
...
import org.sdrc.collect.uphpmis.android.utilities.FileUtils;
import org.sdrc.collect.uphpmis.android.utilities.MediaUtils;
import org.sdrc.collect.uphpmis.android.views.ODKView;
import org.sdrc.collect.uphpmis.android.widgets.QuestionWidget;
...
public class FormEntryActivity extends Activity implements AnimationListener, FormLoaderListener, FormSavedListener,
	...
	private static final int PREVIOUS_DATA = Menu.FIRST + 4;
...
//	private boolean wantToSeePreviousData = false;
				...
				if (getContentResolver().getType(uri).equals(InstanceColumns.CONTENT_ITEM_TYPE)) {
						...
						try {
							...
							if (instanceCursor.getCount() != 1) {
								...
								instancePath = instanceCursor
										.getString(instanceCursor.getColumnIndex(InstanceColumns.INSTANCE_FILE_PATH));
								...
								// Collect.getInstance().setCurrentFormId(jrFormId);
						...
						try {
							...
							if (formCursor.getCount() == 1) {
								...
								this.createErrorDialog(
										getString(R.string.parent_form_not_present, jrFormId) + ((jrVersion == null)
					...
					try {
						...
						if (c.getCount() != 1) {
							...
							String currentFormId = c.getString(c.getColumnIndex("jrFormId"));
							Collect.getInstance().setCurrentFormId(currentFormId);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		CompatibilityUtils.setShowAsAction(
				menu.add(0, MENU_PREFERENCES, 1, R.string.general_preferences).setIcon(R.drawable.ic_menu_preferences),
				MenuItem.SHOW_AS_ACTION_NEVER);
		return true;
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case MENU_HIERARCHY_VIEW:
			...
			i.putExtra("Action", "EditSaved");// Avinash
		...
		case FormEntryController.EVENT_END_OF_FORM:
			...
			((Button) endView.findViewById(R.id.save_exit_button)).setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					/*
					 * Please ensure the following functionality is working fine, before uncommenting the following lines
					 * functionality: in the edit saved form after clicking on "gotoEnd" button and finalize the form, 
					 * the form should go to send finalize form and the control should go to mainmenu activity
					 */
					//goToMainMenu();
				}
			});
	...
	private void goToMainMenu(){
		Intent intent = new Intent(FormEntryActivity.this,MainMenuActivity.class);
//		intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
	...
	private void removeTempInstance() {
		...
		/*
		 * if(Collect.getInstance().getWantToSeePreviousData()) return;
		 */
		...
		if (erase) {
			...
			Log.i(t, "removed from content providers: " + images + " image files, " + audio + " audio files," + " and "
					+ video + " video files.");
	...
	public void savingComplete(SaveResult saveResult) {
		...
		case FormEntryController.ANSWER_REQUIRED_BUT_EMPTY:
			...
			// an answer constraint was violated, so we need to display the
			// proper toast(s)
			// if constraint behavior is on_swipe, this will happen if we do a
			// 'swipe' to the next question
			if (constraint_behavior.equals(PreferencesActivity.CONSTRAINT_BEHAVIOR_ON_SWIPE))
				next();
			// otherwise, we can get the proper toast(s) by saving with
			// constraint check
			else
	...
	private void sendSavedBroadcast() {
		Intent i = new Intent();
		i.setAction("org.sdrc.collect.uphpmis.android.FormSaved");
